...
for (i in 1:nrow(reference_panel_E)){
  var_aa_st1 <- reference_panel_E[i,5]
  for (z in 1:nrow(stable3)){
    var_aa_st3 <- stable3[z,3]
    if (var_aa_st1 == var_aa_st3){
      reference_panel_E[i,7] <- stable3[z,4] 
    }
  }
}
...
#SUPPLEMENTARY TABLE 6: Calculation for Documented variants (BRCAExchange)
...
stable9_form <- function(df_variants_reference_panel,my_data, assay_classes, Espec_Sensi, stable2){
  ...
  stable9 <- cbind(stable9, stable2[,1:2],whole_variants, df_variants_reference_panel[,3],
